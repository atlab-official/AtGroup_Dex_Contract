...
/*
 * This contract is a esArr Usage (plugin) that can boost spNFTs' yield (staking positions on BasePool) when it
 * receives allocations from the esArrToken contract
 */
contract AtComponentBooster is Ownable, ReentrancyGuard, IesARRUsage, IAtComponetBooster {
  using SafeMath for uint256;
  using SafeERC20 for IERC20;
  using EnumerableSet for EnumerableSet.UintSet;

  IesARRToken public immutable esArrToken; // esArrToken contract

  uint256 public constant MAX_TOTAL_ALLOCATION_FLOOR = 40000000 ether;
  ...
  // User's position esArr total allocation
  ...
  mapping(address => uint256) private _usersTotalAllocation; // User's esArr total allocation
  mapping(address => uint256) private _poolsTotalAllocation; // Pool's total allocation
  uint256 public totalAllocation; // Contract's total esArr allocation

  bool public forcedDeallocationStatus; // Authorize users to forcibly deallocate everything

  constructor(IesARRToken esArrToken_) {
  ...
  /**
   * @dev Checks if caller is the esArrToken contract
   */
  modifier esArrTokenOnly() {
    require(msg.sender == address(esArrToken), "esArrTokenOnly: caller should be esArrToken");
    _;
  }
  ...
  /**
   * @dev Returns total esArr allocated to this contract by "userAddress"
   */
  ...
  /**
   * @dev Returns total esArr allocated to this contract by "poolAddress"
   */
  ...
  /**
   * @dev Returns allocated esArr to "tokenId" spNFT from "poolAddress" BasePool by "userAddress"
   */
  ...
  /**
   * Allocates "userAddress" user's "amount" of esArr to this usage contract
   * "data" should contain tokenId and poolAddress
   *
   * Can only be called by esArrToken contract, which is trusted to verify amounts
   */
  function allocate(address userAddress, uint256 amount, bytes calldata data) external override nonReentrant esArrTokenOnly {
    (address poolAddress, uint256 tokenId) = abi.decode(data, (address, uint256));
    _allocate(userAddress, poolAddress, tokenId, amount);

    // allocated esArr is added (as boost points) to spNFT
    IBasePool(poolAddress).boost(tokenId, amount);
  }

  /**
   * Deallocates "userAddress" user's "amount" of esArr from this usage contract
   * "data" should contain tokenId and poolAddress
   *
   * Can only be called by esArrToken contract, which is trusted to verify amounts
   */
  function deallocate(address userAddress, uint256 amount, bytes calldata data) external override nonReentrant esArrTokenOnly {
    (address poolAddress, uint256 tokenId) = abi.decode(data, (address, uint256));
    _deallocate(userAddress, poolAddress, tokenId, amount);

    // should only be called if spNFT has not been burned, to avoid having stuck esArr on it
    if(IBasePool(poolAddress).exists(tokenId)) {
      // allocated esArr is removed (as boost points) from the spNFT
      IBasePool(poolAddress).unboost(tokenId, amount);
    }
  }
   ...
   * The pool should remove the allocated esArr (boost points) from its own spNFT when calling this function
   */
  function deallocateAllFromPool(address userAddress, uint256 tokenId) external override nonReentrant {
    ...
    // update user's esArr allocations balance
    esArrToken.deallocateFromUsage(userAddress, amount);
  }

  /**
   * Deallocates msg.sender's "amount" of esArr from esArrToken, without adjusting this contract allocations balances
   *
  ...
  function forceDeallocate() external nonReentrant {
    ...
    uint256 amount = esArrToken.usageAllocations(msg.sender, address(this));

    // update user's esArr allocations balance
    esArrToken.deallocateFromUsage(msg.sender, amount);
  }
  ...
  /**
   * @dev Returns multiplier that should be applied to a spNFT based on its boost points (allocated esArr)
   *
   ...
   * 1% of the pool's allocated esArr
   *
  ...
  /**
   * @dev Allocates "userAddress" user's "amount" of esArr to "tokenId" spNFT of "poolAddress" pool
   */
  ...
  /**
   * @dev Deallocates "userAddress" user's "amount" of esArr allocated to "tokenId" spNFT of "poolAddress" pool
   */
  function _deallocate(address userAddress, address poolAddress, uint256 tokenId, uint256 amount) internal {
    ...
    require(userPositionAllocation >= amount, "deallocate: not enough allocated esArr");
