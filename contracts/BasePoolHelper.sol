...
contract BasePoolHelper is ReentrancyGuard {
  using Address for address;
  using SafeERC20 for IERC20;
  using SafeMath for uint256;

  bytes4 private constant _ERC721_RECEIVED = 0x150b7a02;
  IAtCoreRouter public immutable router;
  address public immutable weth;

  uint256 expectedTokenId;
  address expectedNftPool;

  constructor(IAtCoreRouter router_, address weth_){
    router = router_;
    weth = weth_;
  }
  ...
  function addLiquidityAndCreatePosition(
    ...
    uint256 amountAMin, uint256 amountBMin, uint256 deadline, address to, IBasePool nftPool, uint256 lockDuration
  ...
  function addLiquidityETHAndCreatePosition(
    ...
    address to, IBasePool nftPool, uint256 lockDuration
