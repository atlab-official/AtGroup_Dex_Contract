...
import './AtCorePair.sol';

contract AtCoreFactory is IAtCoreFactory {
    address public owner;
    address public feePercentOwner;
    address public setStableOwner;
    address public feeTo;

    bytes32 public constant INIT_CODE_HASH = keccak256(abi.encodePacked(type(AtCorePair).creationCode));
     ...
     */
    modifier onlyOwner() {
        require(owner == msg.sender, "AtDexFactory: caller is not the owner");
        _;
    }
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'AtDexFactory: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'AtDexFactory: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'AtDexFactory: PAIR_EXISTS'); // single check is sufficient
        // bytes memory bytecode = type(AtCorePair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        // assembly {
        //     pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        // }
        pair = address(new AtCorePair{salt: salt}());
        require(pair != address(0), "AtDexFactory: FAILED");
        AtCorePair(pair).initialize(token0, token1);
    ...
    function setOwner(address _owner) external onlyOwner {
        require(_owner != address(0), "AtDexFactory: zero address");
        emit OwnershipTransferred(owner, _owner);
        owner = _owner;
    }

    function setFeePercentOwner(address _feePercentOwner) external onlyOwner {
        require(_feePercentOwner != address(0), "AtDexFactory: zero address");
    ...
    function setSetStableOwner(address _setStableOwner) external {
        require(msg.sender == setStableOwner, "AtDexFactory: not setStableOwner");
        require(_setStableOwner != address(0), "AtDexFactory: zero address");
    ...
    function setOwnerFeeShare(uint newOwnerFeeShare) external onlyOwner {
        require(newOwnerFeeShare > 0, "AtDexFactory: ownerFeeShare mustn't exceed minimum");
        require(newOwnerFeeShare <= OWNER_FEE_SHARE_MAX, "AtDexFactory: ownerFeeShare mustn't exceed maximum");
    ...
    function setReferrerFeeShare(address referrer, uint referrerFeeShare) external onlyOwner {
        require(referrer != address(0), "AtDexFactory: zero address");
        require(referrerFeeShare <= REFERER_FEE_SHARE_MAX, "AtDexFactory: referrerFeeShare mustn't exceed maximum");
