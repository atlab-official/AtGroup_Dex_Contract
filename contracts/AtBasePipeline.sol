...
/*
 * This contract centralizes AtDex's yield incentives distribution.
 * Pools that should receive those incentives are defined here, along with their allocation.
 * All rewards are claimed from the ARRToken contract.
 */
contract AtBasePipeline is Ownable, IAtBasePipeline {
  ...
  IARRToken private immutable _arrToken; // Address of the ARR token contract
  IAtComponetBooster private _yieldBooster; // Contract address handling yield boosts
  ...
  constructor(
    ...
    require(address(arrToken_) != address(0), "AtDexPipeline: arrToken cannot be set to zero address");
    require(_currentBlockTimestamp() < startTime_ && startTime_ >= arrToken_.lastEmissionTime(), "AtDexPipeline: invalid startTime");
    _arrToken = arrToken_;
    startTime = startTime_; // Must be set with the same time as ArrToken emission start
  }
  ...
  /*
   * @dev Returns ArrToken address
   */
  function arrToken() external view override returns (address) {
    return address(_arrToken);
  }

  /*
   * @dev Returns ArrToken's emission rate (allocated to this contract)
   */
  function emissionRate() public view returns (uint256) {
    return _arrToken.pipelineEmissionRate();
  }

  /**
   * @dev Returns current owner's address
   */
  function owner() public view virtual override(IAtBasePipeline, Ownable) returns (address) {
  ...
  /**
   * @dev Set AtComponentBooster contract's address
   *
   * Must only be called by the owner
   */
  function setYieldBooster(IAtComponetBooster yieldBooster_) external onlyOwner {
   ...
   * Must only be called by the owner
   */
  function add(IBasePool basePool, uint256 allocPoint, bool withUpdate) external onlyOwner {
    address poolAddress = address(basePool);
   ...
   */
  function updatePool(address basePool) external validatePool(basePool) {
    _updatePool(basePool);
  }
  ...
  function _safeRewardsTransfer(address to, uint256 amount) internal returns (uint256 effectiveAmount) {
    uint256 arrBalance = _arrToken.balanceOf(address(this));
  ...
  function _updatePool(address poolAddress) internal {
    ...
    if (allocPoint > 0 && IBasePool(poolAddress).hasDeposits()) {
      // calculate how much ARR rewards are expected to be received for this pool
      ...
      (rewards) = _arrToken.claimPipelineRewards(rewards);
