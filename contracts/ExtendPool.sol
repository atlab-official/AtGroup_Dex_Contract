...
import "./interfaces/IExtendCustomReq.sol";


contract ExtendPool is ReentrancyGuard, Ownable, IPoolOperator {
    using EnumerableSet for EnumerableSet.UintSet;
    using EnumerableSet for EnumerableSet.AddressSet;
    using SafeERC20 for IERC20;
    using SafeERC20 for IesARRToken;
    ...
    struct Settings {
        ...
        string description; // Project's description for this ExtendPool
    }
    ...
    IExtendPoolFactory public factory; // ExtendPoolFactory address
    IARRToken public arrToken; // ARRToken contract
    IesARRToken public esArrToken; // esArrToken contract
    IBasePool public basePool; // BasePool contract
    IExtendCustomReq public customReqContract; // (optional) external contracts allow to handle custom requirements

    uint256 public creationTime; // Creation time of this ExtendPool

    bool public published; // Is ExtendPool published
    uint256 public publishTime; // Time at which the ExtendPool was published
    ...
    constructor(
        IARRToken arrToken_, IesARRToken esArrToken_, address owner_, IBasePool basePool_,
        IERC20 rewardsToken1_, IERC20 rewardsToken2_, Settings memory settings_
    ) {
        require(address(arrToken_) != address(0) && address(esArrToken_) != address(0) && owner_ != address(0)
            && address(basePool_) != address(0) && address(rewardsToken1_) != address(0), "zero address");
        ...
        factory = IExtendPoolFactory(msg.sender);
    ...
    modifier isValidBasePool(address sender) {
        require(sender == address(basePool), "invalid BasePool");
        _;
    }
     ...
     */
    function onERC721Received(address /*operator*/, address from, uint256 tokenId, bytes calldata /*data*/) external override nonReentrant isValidBasePool(msg.sender) returns (bytes4) {
        ...
        basePool.approve(from, tokenId);
        return _ERC721_RECEIVED;
    }

    /**
     * @dev Withdraw a position from the ExtendPool
     *
    ...
    function withdraw(uint256 tokenId) external virtual nonReentrant {
        ...
        basePool.safeTransferFrom(address(this), msg.sender, tokenId);

        emit Withdraw(msg.sender, tokenId, amount);
    }

    /**
     * @dev Withdraw a position from the ExtendPool without caring about rewards, EMERGENCY ONLY
     *
    ...
    /**
     * @dev Harvest pending ExtendPool rewards
     */
     ...
     * "to" can be set to this address only if this contract is allowed to transfer esArr
     */
    function onNFTHarvest(address operator, address to, uint256 tokenId, uint256 arrAmount, uint256 esArrAmount) external override isValidBasePool(msg.sender) returns (bool) {
        address owner = tokenIdOwner[tokenId];
        require(operator == owner, "not allowed");

        // if not whitelisted, the ExtendPool can't transfer any esArr rewards
        require(to != address(this) || esArrToken.isTransferWhitelisted(address(this)), "cant handle rewards");

        // redirect rewards to position's previous owner
        if (to == address(this)) {
            arrToken.safeTransfer(owner, arrAmount);
            esArrToken.safeTransfer(owner, esArrAmount);
        }
     ...
     */
    function onNFTAddToPosition(address operator, uint256 tokenId, uint256 amount) external override nonReentrant isValidBasePool(msg.sender) returns (bool) {
    ...
    /**
     * @dev Transfer ownership of this ExtendPool
     *
     * Must only be called by the owner of this contract
     */
    function transferOwnership(address newOwner) public override onlyOwner {
        _setExtendPoolOwner(newOwner);
    ...
    function renounceOwnership() public override onlyOwner {
        _setExtendPoolOwner(address(0));
        Ownable.renounceOwnership();
    }

    /**
     * @dev Add rewards to this ExtendPool
     */
    function addRewards(uint256 amountToken1, uint256 amountToken2) external nonReentrant {
        ...
        // get active fee share for this ExtendPool
        uint256 feeShare = factory.getExtendPoolFee(address(this), owner());
    ...
    /**
     * @dev Withdraw rewards from this ExtendPool
     *
     * Must only be called by the owner
     * Must only be called before the publication of the Extend Pool
     */
    ...
    function setCustomReqContract(address contractAddress) external onlyOwner {
        ...
        customReqContract = IExtendCustomReq(contractAddress);

        emit SetCustomReqContract(contractAddress);
    }

    /**
     * @dev Set requirements that positions must meet to be staked on this Extend Pool
     *
     * Must only be called by the owner
     */
     ...
     *
     * Must only be called by the owner
     * Extend duration can only be extended once already published
    ...
    /**
     * @dev Publish the Extend Pool
     *
     * Must only be called by the owner
     */
    function publish() external onlyOwner {
        require(!published, "published");
        // this extendPool is Stale
        ...
        factory.publishExtendPool(address(basePool));
    ...
    /**
     * @dev Updates rewards states of this Extend Pool to be up-to-date
     */
    ...
    /**
     * @dev Add a user's deposited amount into this Extend Pool
     */
    ...
    /**
     * @dev Check whether a position with "tokenId" ID is meeting all of this Extend Pool's active requirements
     */
  ...
  */
    ...
    function _getStackingPosition(uint256 tokenId) internal view returns (uint256 amount, uint256 startLockTime, uint256 lockDuration) {
        (amount,, startLockTime, lockDuration,,,,,,,) = basePool.getStakingPosition(tokenId);
    }

    function _setExtendPoolOwner(address newOwner) internal {
        factory.setExtendPoolOwner(owner(), newOwner);
    }
