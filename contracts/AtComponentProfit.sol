...
import "./interfaces/IAtComponetProfit.sol";
import "./interfaces/IesARRUsage.sol";


/*
 * This contract is used to distribute dividends to users that allocated esArr here
 *
 ...
 * This distribution slot is the source of the dividends distribution to esArr allocators during the current cycle
 *
 * This transfer from the pending slot to the distribution slot is based on cycleDividendsPercent and CYCLE_PERIOD_SECONDS
 *
 */
contract AtComponentProfit is Ownable, ReentrancyGuard, IesARRUsage, IAtComponetProfit {
  ...
  address public immutable esArrToken; // esArrToken contract

  mapping(address => uint256) public usersAllocation; // User's esArr allocation
  ...
  constructor(address esArrToken_, uint256 startTime_) {
    require(esArrToken_ != address(0), "zero address");
  ...
  modifier esArrTokenOnly() {
    require(msg.sender == esArrToken, "esArrTokenOnly: caller should be esArrToken");
    _;
  }
  ...
  /**
   * Allocates "userAddress" user's "amount" of esArr to this dividends contract
   *
   * Can only be called by esArrToken contract, which is trusted to verify amounts
   * "data" is only here for compatibility reasons (IesARRUsage)
   */
  ...
  /**
   * Deallocates "userAddress" user's "amount" of esArr allocation from this dividends contract
   *
   * Can only be called by esArrToken contract, which is trusted to verify amounts
   * "data" is only here for compatibility reasons (IesARRUsage)
   */
  ...
  */
  ...
  function _updateDividendsInfo(address token) internal {
    ...
    // if no esArr is allocated or initial distribution has not started yet
  ...
  function _harvestDividends(address token) internal {
    ...
    uint256 useresArrAllocation = usersAllocation[msg.sender];
    uint256 pending = user.pendingDividends.add(
      useresArrAllocation.mul(accDividendsPerShare).div(1e18).sub(user.rewardDebt)
    );

    user.pendingDividends = 0;
    user.rewardDebt = useresArrAllocation.mul(accDividendsPerShare).div(1e18);
